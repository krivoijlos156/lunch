...
public interface CrudVoteRepository extends JpaRepository<Vote, Integer> {
    ...
    @Query("SELECT v FROM Vote v WHERE v.userId=:userId AND v.dateTime >= :startDate AND v.dateTime < :endDate ORDER BY v.dateTime DESC")
    List<Vote> getBetweenDateTime(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, @Param("userId") int userId);
}